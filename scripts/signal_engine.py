...
def fetch_data():
    ...
    calc_cols = ['close', 'rsi', 'sma_50', 'sma_200', 'ema_20', 'macd_macd', 'macd_signal', 
                 'stoch_k', 'stoch_d', 'bb_upper', 'bb_lower', 'atr', 'adx', 'volume', 
                 'average_volume_30d', 'return_on_equity', 'gross_margin', 'net_margin', 
                 'return_on_assets', 'debt_to_equity', 'dividends_yield']
...
def calculate_indicators(df):
    ...
    if 'volume' in df.columns and 'average_volume_30d' in df.columns:
        ...
        df['rvol'] = np.nan
        df['is_liquid'] = True # Default to true if we can't tell, or false? Let's say true to not suppress.
...
def compute_signal_score(row):
    ...
    rsi = row.get('rsi', 50)
    if rsi < 35:
        bull += 2; flags.append('RSI Oversold')
    elif rsi > 65:
        bear += 2; flags.append('RSI Overbought')

    # --- Sector-Relative RSI ---
    rel_rsi = row.get('relative_rsi', 0)
    if rel_rsi > 10 and rsi < 55:
        bull += 2; flags.append('Sector RSI Strong')
    elif rel_rsi < -10 and rsi > 45:
        bear += 2; flags.append('Sector RSI Weak')
    ...
    if adx > 20 and sma50 and sma200:
        if sma50 > sma200:
            bull += 3; flags.append('ADX Golden Cross')
        elif sma50 < sma200:
            bear += 3; flags.append('ADX Death Cross')
    ...
    if macd is not None and sig is not None:
        if macd > sig:
            bull += 2; flags.append('MACD Bullish')
        else:
            bear += 2; flags.append('MACD Bearish')

    # --- BB Position ---
    bbp = row.get('bb_position')
    if pd.notna(bbp):
        if bbp < 0.2:
            bull += 1; flags.append('BB Lower Zone')
        elif bbp > 0.8:
            bear += 1; flags.append('BB Upper Zone')

    # --- RVOL ---
    rvol = row.get('rvol', 0)
    if pd.notna(rvol) and rvol > 1.5:
        if row.get('is_green'):
            bull += 2; flags.append('RVOL Spike + Green')
        elif row.get('is_red'):
            bear += 2; flags.append('RVOL Spike + Red')
    ...
    if row.get('is_squeeze', False):
        ...
        if close and up and close > up and rvol > 1.5:
            bull += 3; flags.append('Squeeze Breakout UP')
        elif close and low and close < low and rvol > 1.5:
            bear += 3; flags.append('Squeeze Breakout DOWN')
    ...
    if sk is not None and sd is not None:
        if sk < 20 and sd < 20:
            bull += 1; flags.append('Stoch Oversold')
        elif sk > 80 and sd > 80:
            bear += 1; flags.append('Stoch Overbought')

    # --- TV Rating ---
    tr = row.get('technical_rating', 0) or 0
    if tr >= 0.5:
        bull += 3; flags.append('TV Strong Buy')
    elif tr >= 0.1:
        bull += 1; flags.append('TV Buy')
    elif tr <= -0.5:
        bear += 3; flags.append('TV Strong Sell')
    elif tr <= -0.1:
        bear += 1; flags.append('TV Sell')
...
def detect_perfect_entry(row):
    ...
    # Golden Pullback
    if (sma50 > sma200 and close > sma200 and close < sma50 and 
        rsi < 45 and bbp < 0.25 and rvol < 0.8):
...
def compute_quality_score(row):
    ...
    debt_score = max(0, 1 - (de / 150)) # tvscreener returns percentage, e.g. 150.0 for 1.5
    score += debt_score * 10
...
def main():
    try:
        ...
        output_cols = [
            'ticker', 'name', 'close', 'price_change_pct', 'bull_score', 'bear_score', 
            'conviction', 'perfect_entry', 'signal_flags', 
            'quality_score', 'quality_grade', 'is_liquid', 
            'rvol', 'bb_position', 'relative_rsi', 'is_squeeze',
            'rsi', 'sma_50', 'sma_200'
        ]
