...
app.get('/api/market-signals', async (req, res) => {
  try {
    ...
    if (fs.existsSync(signalsPath)) {
      ...
      // If file doesn't exist, try to run it once and return "Processing"
      runSignalEngine().catch(console.error);
      res.status(404).json({
        error: 'Signals not found',
        message: 'Signal engine is being initialized. Please refresh in a few seconds.'
      });
    }
  } catch (err) {
...
app.post('/api/trigger-signals', async (req, res) => {
  try {
    ...
    if (result.success) {
      res.json({ success: true, message: 'Signal engine executed successfully' });
    } else {
      res.status(500).json({
        success: false,
...
// Initial run of signal engine if file doesn't exist
const signalsPath = path.join(process.cwd(), 'market_signals.json');
if (!fs.existsSync(signalsPath)) {
  console.log('Signal engine data not found. Running engine once...');
  runSignalEngine().catch(console.error);
}

// Helper to run the python signal engine
async function runSignalEngine() {
  ...
  for (const cmd of commands) {
    try {
      ...
      console.log(`Signal Engine (${cmd}) Output:`, stdout);
      if (stderr) console.warn(`Signal Engine (${cmd}) Stderr:`, stderr);
      return { success: true, stdout, stderr };
    } catch (error: any) {
      lastError = error.message;
      lastStderr = error.stderr || '';
      console.log(`Failed to run with ${cmd}, trying next...`);
    }
  }

  console.error('All python commands failed to run the signal engine.');
  return { success: false, error: lastError, stderr: lastStderr };
}
