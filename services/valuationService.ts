...
export interface ScreenFilters {
    ...
    signal?: 'overvalued' | 'fair' | 'undervalued';
    sector?: string;
    sortBy?: 'pe' | 'garpScore' | 'divYield' | 'pe_percentile' | 'pegRatio';
    sortOrder?: 'asc' | 'desc';
}
...
export class ValuationService {
    ...
    static async getValuationRanges(ticker: string): Promise<ValuationMetrics | null> {
        ...
        // Get PE values (filter out undefined / non‑positive)
    ...
    static async getMultipleValuations(tickers: string[]): Promise<ValuationMetrics[]> {
        const metrics = await Promise.all(tickers.map(t => this.getValuationRanges(t)));
        return metrics.filter((m): m is ValuationMetrics => m !== null);
    }
    ...
    static async screenByMetrics(filters: ScreenFilters): Promise<ValuationMetrics[]> {
        ...
        // Calculate percentiles for sector (defensively)
        if (results.length > 0) {
            const pePopulation = results
                .map(r => r.currentPE)
                .filter((pe): pe is number => pe !== undefined && pe > 0);
            ...
            results = results.map(r => ({
                ...r,
                pePercentile:
                    r.currentPE !== undefined &&
                    r.currentPE > 0 &&
                    pePopulation.length > 0
                        ? calculatePercentile(r.currentPE, pePopulation)
                        : undefined
            }));
        }
