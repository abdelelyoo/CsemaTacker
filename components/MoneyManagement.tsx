...
export const MoneyManagement: React.FC = () => {
  ...
  // AI Portfolio Analyst State (Merged from AIInsights)
  ...
  const runPortfolioAnalysis = async () => {
    ...
    try {
      ...
      setAnalysisError('Failed to generate analysis. Please check your API key.');
    } finally {
  ...
  // Auto-run Portfolio Analysis (from AIInsights)
  useEffect(() => {
    if (!analysis && !loadingAnalysis && !analysisError) {
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-20 md:pb-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* AI Portfolio Analyst (Merged from AIInsights) */}
          <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm mt-8">
            <div className="flex justify-between items-start mb-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900">AI Portfolio Analyst</h3>
                  <div className="flex items-center space-x-2">
                    <p className="text-slate-500 text-sm">Comprehensive Portfolio Health Audit</p>
                    {analysisDate && (
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg bg-slate-900 text-white hover:bg-slate-800 transition-all ${loadingAnalysis ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                <RefreshCcw size={16} className={loadingAnalysis ? 'animate-spin' : ''} />
                <span>{loadingAnalysis ? 'Analyzing...' : 'Refresh Audit'}</span>
              </button>
            </div>

            <div className={`prose prose-slate prose-sm max-w-none p-6 rounded-xl border border-slate-100 ${loadingAnalysis ? 'bg-slate-50' : 'bg-white'}`}>
              {loadingAnalysis ? (
                <div className="flex flex-col items-center justify-center space-y-4 py-12">
                  <div className="w-8 h-8 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin"></div>
                  <p className="text-slate-500 animate-pulse font-medium">Conducting holistic portfolio audit...</p>
                </div>
              ) : analysisError ? (
                ...
                <div className="text-center py-12 text-slate-400 italic">
                  No analysis available. Click refresh to begin the portfolio audit.
                </div>
              )}
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-slate-100">
              <div className="flex items-start gap-2 text-slate-400">
                ...
                <p className="text-[10px] leading-relaxed">
                  Insights generated by Gemini AI. This audit covers sector concentration, fee drag, execution quality, and risk factors. Educational purposes only.
                </p>
              </div>
            </div>
          </div>
